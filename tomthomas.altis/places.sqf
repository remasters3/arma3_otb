private _centerWorld =  _this Select 0;
	...
	_pCount = 0;
	_SidePos = [
	[26662.9,24535.5,0], //WEST
	[9095.43,21353.3,0], //EAST
	[20830.4,7166.33,0], // Resistance
	[14754.6,16646.6,0] //Civ
];
	//while {_pCount < 360} Do {
	//_pos = [_centerGround, 7500, _pCount] call BIS_fnc_relPos; //[([_LocObjectT, 1000, _pCount] call BIS_fnc_relPos),1,200, 20, 0, 60 * (pi / 180), 0, []] call BIS_fnc_findSafePos;
	//_pCount = _pCount+120;
...
if (isDedicated or isServer) then { _offset = 0;
	//_centerGround = createVehicle ["VR_Area_01_square_4x4_grey_F", [(_centerWorld Select 0),(_centerWorld Select 1),0], [], 0, "FORM"];


	//deleteVehicle _centerGround;
