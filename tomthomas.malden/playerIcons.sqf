...
If (Side player == West      ) Then {_iconColour = [0,0.3,0.6,0.6];_trgt = GetMarkerPos "marker_b_helipad";_dst = player distance _trgt;/*_txt = format ["Drop Zone - %1m",floor _dst];*/_icon = "\A3\EditorPreviews_F_Orange\Data\CfgVehicles\Banner_01_NATO_F.jpg";};
If (Side player == East      ) Then {_iconColour = [0.5,0,0,0.6];_trgt = GetMarkerPos "marker_r_helipad";_dst = player distance _trgt;/*_txt = format ["Drop Zone - %1m",floor _dst];*/_icon = "\A3\EditorPreviews_F_Orange\Data\CfgVehicles\Banner_01_CSAT_F.jpg";};
If (Side player == Resistance) Then {_iconColour = [0,0.5,0,0.6];_trgt = GetMarkerPos "marker_g_helipad";_dst = player distance _trgt;/*_txt = format ["Drop Zone - %1m",floor _dst];*/_icon = "\A3\EditorPreviews_F_Orange\Data\CfgVehicles\Banner_01_AAF_F.jpg"; };
...
if (ShowFriendly) then {
	_dst = player distance _trgt;
	_txt = format ["%1m",floor _dst];
	drawIcon3D [_icon, [0.4,0,0.5,0.6],_trgt, 0.5, 0.5, 1,_txt, 0, 0.04, "RobotoCondensedLight","center",true];
	{
			...
			if ("evacleader" in allVariables _x) then {
				...
				if (_x getVariable "evacleader") then {
						...
						if ((vehicle _x) == _x) then {
							...
							private _icon = "\a3\3den\data\cfgwaypoints\load_ca.paa";//"\a3\3den\data\cfg3den\object\iconcommander_ca.paa"; //'iconMan';
...
{
	if (side player != side _x) then {
		If (Side _x == West      ) Then {_iconColour = [0,0.3,0.6,0.6];};
		If (Side _x == East      ) Then {_iconColour = [0.5,0,0,0.6];};
		If (Side _x == Resistance) Then {_iconColour = [0,0.5,0,0.6];};
