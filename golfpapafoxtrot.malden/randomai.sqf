...
_sourceObjectT = createVehicle ["VR_Area_01_square_4x4_grey_F", [(_centerWorld Select 0),(_centerWorld Select 1),0], [], 0, "FORM"];
...
if (_Side == west) Then {_vehs = _WestLightVeh;
						 ...
						 _towermodel = "Land_Cargo_Patrol_V1_F";
						 };
if (_Side == east) Then {_vehs = _EastLightVeh;
						 ...
						 _towermodel = "Land_Cargo_Patrol_V2_F";
						 };						 
if (_Side == resistance) Then {_vehs = _ResLightVeh;
							   ...
							   _towermodel = "Land_Cargo_Patrol_V3_F";
							   };
							   
if ((count Allunits) < _maxunits) Then {				   
    private _pos = SelectRandom _places;
    private _exec = SelectRandom [3];
	private _exek = SelectRandom [0];
    private _safePos =  [_pos,10,110, 10, 0, 60 * (pi / 180), 0, []] call BIS_fnc_findSafePos;
	private _cpos = [(_centerWorld Select 0),(_centerWorld Select 1),0];

    if (_exec == 1) Then {[[_safePos],_Side,_troops,0,_timeout,_towermodel] Call GPF_fnc_GuardTowers;};
    if (_exec == 2) Then {[_safePos,_pos,5,10,_Side,_troops,0,_timeout,[],_transport] Call GPF_fnc_barraks;};
    if (_exec == 3) Then {[_pos,_HeliModel,_Side,_troops,_timeout,FALSE] Call GPF_fnc_TroopDrop;};
	
    if (_exek == 1) Then {[_cpos ,500,45,360,_Side ,(SelectRandom _vehs),30] Call GPF_fnc_patrol;};
    if (_exek == 2) Then {_sourceObjectT SetPos _cpos;_ewu = [_sourceObjectT,300,45,360,_Side,_troops] call GPF_fnc_enemyWave;[_ewu,_timeout]Spawn {_ewu = _this Select 0; _timeout = _this Select 1;Sleep _timeout;{deleteVehicle _x;} foreach _ewu;};};
    if (_exek == 3) Then {_sourceObjectT SetPos _cpos;_ewu = [_sourceObjectT,500,45,360,_Side,_vehs] call GPF_fnc_enemyWaveVeh;[_ewu,_timeout]Spawn {_ewu = _this Select 0; _timeout = _this Select 1;Sleep _timeout;{_veh = _x; {deleteVehicle _x;} Foreach Crew _veh; deleteVehicle _veh;} foreach _ewu;};};
    if (_exek == 4) Then {[_cpos ,1500,90,360,_Side ,( SelectRandom _PlaneModel),30] Call GPF_fnc_patrol;};
};
